...
export default function InputFlow() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (mode === 'income') {
            addIncome({
                id, date, amount: val.toString(), description: description || 'Income',
                category: category || 'General', receiptId: '', client: '', currency: 'NGN', exchangeRate: 1
            });
        } else {
            addExpense({
                id, date, amount: val.toString(), description: description || 'Expense',
                category: category || 'General', receiptId: '', deductible: true
            });
        }
            ...
            <div className="flex gap-2 mb-6 bg-black/40 p-1 rounded-full border border-white/5">
                <button
                    ...
                    className={cn(
                        ...
                        mode === 'income' ? "bg-emerald-600 text-white shadow-lg" : "text-gray-400 hover:text-white"
                    )}
                >
                    <Plus size={16} /> INCOME
                </button>
                <button
                    ...
                    className={cn(
                        "flex-1 py-3 rounded-full text-sm font-bold tracking-wide transition-all flex items-center justify-center gap-2",
                        mode === 'expense' ? "bg-red-600 text-white shadow-lg" : "text-gray-400 hover:text-white"
                    )}
                >
