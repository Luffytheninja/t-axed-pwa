      ...
      {isOpen && (
        <div className="px-6 pb-6 space-y-4">
          {schedule.map((row, i) => (
            <div
              key={i}
              className="bg-black/40 rounded-2xl p-4 border border-white/5 flex justify-between items-center group hover:border-white/10 transition-colors"
            >
          ...
          <div className="mt-4 p-4 bg-red-500/5 rounded-2xl border border-red-500/10 flex gap-3">
            ...
            <p className="text-[10px] text-red-200/60 leading-relaxed italic">
              Strict adherence to remittance dates is mandatory to avoid the{' '}
              <span className="text-red-400 font-bold">₦100,000 NTAA penalty</span> under the 2026
              guidelines.
            </p>
          </div>
        </div>
