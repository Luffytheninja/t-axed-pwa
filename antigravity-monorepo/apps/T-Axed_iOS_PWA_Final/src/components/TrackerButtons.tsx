...
export default function TrackerButtons({
    showIncomeTracker, setShowIncomeTracker,
    showExpenseTracker, setShowExpenseTracker,
        ...
        <div className="grid grid-cols-2 gap-4">
            ...
            >
                <div className="flex flex-col h-full relative z-10">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 shadow-sm transition-colors duration-300 ${showIncomeTracker ? 'bg-blue-600 text-white' : 'bg-emerald-100 text-emerald-600'}`}>
                        <Receipt className="w-5 h-5" />
                    </div>
                    <div>
                        <div className="flex items-center gap-1">
                            <h3 className="text-lg font-bold text-gray-900 leading-tight">Revenue</h3>
                            {showIncomeTracker && <ArrowUpRight className="w-3 h-3 text-blue-600 animate-bounce" />}
                        </div>
                        <p className={`text-[10px] font-black uppercase tracking-wider mt-1 transition-colors ${showIncomeTracker ? 'text-blue-600' : 'text-gray-400'}`}>
            ...
            >
                <div className="flex flex-col h-full relative z-10">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 shadow-sm transition-colors duration-300 ${showExpenseTracker ? 'bg-red-600 text-white' : 'bg-red-100 text-red-600'}`}>
                        <FileText className="w-5 h-5" />
                    </div>
                    <div>
                        <div className="flex items-center gap-1">
                            <h3 className="text-lg font-bold text-gray-900 leading-tight">Outflow</h3>
                            {showExpenseTracker && <ArrowDownRight className="w-3 h-3 text-red-600 animate-bounce" />}
                        </div>
                        <p className={`text-[10px] font-black uppercase tracking-wider mt-1 transition-colors ${showExpenseTracker ? 'text-red-600' : 'text-gray-400'}`}>
