            ...
            {showQuarterly && (
                <div className="space-y-4">
                    {getQuarterlyPayments().map((payment, idx) => (
                        <div key={idx} className="ios-input-group !bg-white/50 hover:border-indigo-200 transition-all cursor-default">
                            <div className="p-5 flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <div className={`w-2.5 h-2.5 rounded-full shadow-sm ${payment.status === 'Due' ? 'bg-red-500 animate-pulse' : 'bg-gray-300'}`} />
                                    <div>
                                        <h3 className="text-sm font-bold text-gray-900 leading-none mb-1">{payment.quarter}</h3>
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{payment.dueDate}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-lg font-bold text-gray-900 leading-none mb-1">{formatCurrency(payment.amount)}</p>
                                    <p className={`text-[10px] font-black uppercase tracking-widest ${payment.status === 'Due' ? 'text-red-600' : 'text-gray-400'}`}>
                                        {payment.status}
                                    </p>
                                </div>
                            </div>
                    ...
                    <div className="bg-red-50/50 border-l-4 border-red-500 p-4 rounded-xl mt-6">
                        <p className="text-[11px] font-medium text-red-800 leading-relaxed">
                            ...
                            Payments must be received by 11:59 PM on the due date to avoid the mandatory ₦100,000 NTAA penalty.
                        </p>
                    </div>
                </div>
            )}
