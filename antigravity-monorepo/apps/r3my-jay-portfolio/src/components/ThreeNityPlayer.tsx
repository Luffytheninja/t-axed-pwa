            ...
            <div className="flex items-center gap-6 overflow-x-auto pb-4 border-b border-accent/10 no-scrollbar">
                {eras.map((era) => (
                    <button
                        ...
                        className={`text-xs font-bold tracking-[0.2em] uppercase transition-all whitespace-nowrap ${activeEra === era ? "text-accent border-b border-accent pb-2" : "text-cloudDancer/40 hover:text-cloudDancer"
                            }`}
                    >
                        {era}
                    </button>
                    ...
                    <div className="space-y-6">
                        <div className="flex justify-between items-end">
                            <div>
                                ...
                                <h3 className="text-3xl font-black kinetic-text tracking-tighter uppercase">{currentTrack.title}</h3>
                                <p className="text-xs text-cloudDancer/60 font-medium italic">{currentTrack.era} — {currentTrack.mood}</p>
                            </div>
                            <div className="text-right">
                                <p className="text-[10px] text-accent/60 font-bold uppercase">{currentTrack.streamCount} REACH</p>
                ...
                <div className="lg:col-span-7 flex flex-col gap-2">
                    {filteredTracks.map((track, index) => (
                        <motion.div
                            ...
                            className={`group flex items-center justify-between p-4 border border-transparent hover:border-accent/20 cursor-pointer transition-all ${currentTrack.id === track.id ? "bg-accent/5 border-accent/20" : "hover:bg-accent/2"
                                }`}
                        >
                            <div className="flex items-center gap-6">
                                <span className={`text-xs font-bold w-4 ${currentTrack.id === track.id ? "text-accent" : "text-cloudDancer/20"}`}>
                                    {String(index + 1).padStart(2, '0')}
                                </span>
                                <div className="relative w-12 h-12 overflow-hidden bg-charcoal">
                                    <img src={track.art} alt={track.title} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all" />
                                    {currentTrack.id === track.id && (
                                        <div className="absolute inset-0 bg-accent/40 flex items-center justify-center">
                                            <div className="flex gap-1 h-3 items-end">
                                                <div className="w-0.5 bg-background animate-bounce" style={{ animationDelay: '0s' }} />
                                                <div className="w-0.5 bg-background animate-bounce" style={{ animationDelay: '0.2s' }} />
                                                <div className="w-0.5 bg-background animate-bounce" style={{ animationDelay: '0.4s' }} />
                                            </div>
                                        </div>
                                    )}
                                </div>
                                <div>
                                    <h4 className={`text-sm font-bold tracking-wider uppercase transition-colors ${currentTrack.id === track.id ? "text-accent" : "group-hover:text-accent"}`}>
                                        {track.title}
                                    </h4>
                                    <p className="text-[10px] text-cloudDancer/40 font-bold uppercase tracking-[0.1em]">{track.mood} // {track.era}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-8">
                                ...
                                <span className="text-[10px] font-bold text-cloudDancer/40 hidden md:block">{track.duration}</span>
                            </div>
                        </motion.div>
                    ))}
                </div>
