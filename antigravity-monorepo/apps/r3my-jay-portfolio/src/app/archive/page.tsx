...
const socialLinks = [
  ...
  },
  {
    ...
    handle: '@r3myjay',
    icon: Instagram,
    url: 'https://instagram.com/r3myjay',
    color: '#f3f3ed',
  },
  {
    name: 'YOUTUBE',
    handle: 'r3my jay Official',
    icon: Youtube,
    url: 'https://www.youtube.com/channel/UC3_aQA0Nne7FEo2SA30xRCQ',
    color: '#ff0000',
  },
  {
    name: 'GENIUS',
    handle: 'r3my jay',
    icon: Music,
    url: 'https://genius.com/artists/R3my-jay',
    color: '#ffff00',
  },
];
        ...
        <div className="lg:col-span-6 space-y-24">
          <section className="space-y-8">
            ...
            <p className="text-cloudDancer/60 text-sm font-light leading-relaxed">
              The primary communication channel for The 3NITY. Early unreleased demos, first-look
              merch, and secret show coordinates sent directly to your neural network.
            </p>
          ...
          <section className="space-y-8">
            ...
            <div className="space-y-4 border-l border-accent/10 pl-6">
              {coordinates.map((coord) => (
                <div key={coord.id} className="relative group">
                  <div
                    className={`absolute -left-[27px] top-1 w-2 h-2 rotate-45 border ${coord.active ? 'bg-accent border-accent' : 'bg-background border-accent/30'}`}
                  />
                  <div className="flex justify-between items-start">
                    <div>
                      <h3
                        className={`text-sm font-bold uppercase tracking-widest ${coord.active ? 'text-accent' : 'text-cloudDancer/40'}`}
                      >
                        {coord.location}
                      </h3>
                      <p className="text-[10px] text-cloudDancer/40 font-medium uppercase italic">
                        {coord.event}
                      </p>
                    </div>
            ...
            <div className="aspect-[21/9] bg-charcoal border border-accent/10 relative overflow-hidden grayscale">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-[10px] font-bold tracking-[0.5em] text-accent/20 uppercase select-none">
                  SCANNING LAGOS SEC...
                </span>
              </div>
            </div>
            ...
            <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-[9px] font-bold tracking-widest text-accent uppercase">
                    Requester
                  </label>
                  <input
                    type="text"
                    className="w-full bg-charcoal border border-accent/10 p-3 text-xs focus:outline-none focus:border-accent"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[9px] font-bold tracking-widest text-accent uppercase">
              ...
              <div className="space-y-2">
                ...
                <textarea
                  rows={4}
                  className="w-full bg-charcoal border border-accent/10 p-3 text-xs focus:outline-none focus:border-accent resize-none"
                ></textarea>
              </div>
          ...
          <section className="space-y-12">
            ...
            <div className="grid grid-cols-2 gap-4">
              {socialLinks.map((link) => (
                ...
                >
                  <link.icon
                    className="text-cloudDancer/20 group-hover:text-accent transition-colors"
                    size={20}
                  />
                  <div>
                    <h4 className="text-[10px] font-bold tracking-widest uppercase text-accent mb-1">
                      {link.name}
                    </h4>
                    <p className="text-sm font-bold kinetic-text tracking-tight uppercase opacity-60 group-hover:opacity-100">
                      {link.handle}
                    </p>
                  </div>
                </a>
