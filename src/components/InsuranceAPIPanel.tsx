...
const INITIAL_PROVIDERS: InsuranceProvider[] = [
    { id: 'axa', name: 'AXA Mansard', type: 'Life & Health', api: 'https://api.axamansard.com', status: 'Available', isConnected: false },
    { id: 'custodian', name: 'Custodian Investment', type: 'Life Insurance', api: 'https://api.custodianplc.com', status: 'Available', isConnected: false },
    { id: 'leadway', name: 'Leadway Assurance', type: 'Life & Health', api: 'https://api.leadway.com', status: 'Integration Ready', isConnected: false },
    { id: 'aiico', name: 'AIICO Insurance', type: 'Life Insurance', api: 'https://api.aiicoplc.com', status: 'Available', isConnected: false },
    { id: 'hygeia', name: 'Hygeia HMO', type: 'Health Insurance', api: 'https://api.hygeiahmo.com', status: 'Available', isConnected: false },
    { id: 'reliance', name: 'Reliance HMO', type: 'Health Insurance', api: 'https://api.reliancehmo.com', status: 'Available', isConnected: false }
];
        ...
        <div className="ios-card p-8 mb-8">
            ...
            <div className="grid md:grid-cols-2 gap-4">
                {providers.map((insurer) => (
                    <div
                        key={insurer.id}
                        className={`ios-card border p-5 transition-all duration-200 ${insurer.isConnected
                    ...
                    >
                        <div className="flex items-start justify-between mb-4">
                            <div>
                                ...
                                <p className="text-xs text-indigo-600 font-semibold uppercase tracking-wider mt-1">{insurer.type}</p>
                            </div>
                            {insurer.isConnected ? (
                                ...
                                <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${insurer.status === 'Available' ? 'bg-indigo-100 text-indigo-700' : 'bg-amber-100 text-amber-700'
                                    }`}>
                                    {insurer.status}
                                </span>
                            )}
                        ...
                        {insurer.isConnected ? (
                            <div className="space-y-3 mt-4 animate-in fade-in slide-in-from-bottom-2">
                                <div className="ios-input-group !bg-white/80">
                                    <div className="ios-input-item !border-0 flex justify-between items-center py-2 px-3">
                                        ...
                                        <span className="text-xs font-mono text-gray-900">{insurer.syncedPolicyNumber}</span>
                                    </div>
                                    <div className="ios-input-item !border-t border-gray-100 flex justify-between items-center py-2 px-3">
                                        ...
                                        <span className="font-bold text-green-700">{formatCurrency(insurer.syncedPremium || 0)}</span>
                                    </div>
                                </div>
                            </div>
                        ) : (
                            <div className="mt-4">
                                <button
                                    ...
                                    className={`ios-btn-primary w-full py-2.5 !text-sm ${connectingId === insurer.id
            ...
            <div className="mt-6 bg-blue-50 border-l-4 border-blue-500 p-4 rounded flex gap-3">
                ...
                <p className="text-sm text-blue-800">
                    <strong>API Integration Benefits:</strong> Real-time premium verification prevents tax audit queries.
                    Synced data is automatically stamped with the insurer's digital signature for NTA 2025 compliance.
                </p>
            </div>
        </div>
    );
