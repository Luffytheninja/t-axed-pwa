        ...
        <div className="ios-card p-8 mb-8">
            <div className="flex items-center justify-between mb-8">
                ...
                <button
                    ...
                    className="ios-btn-secondary px-6"
                >
                ...
                <div className="space-y-8">
                    <div className="ios-input-group p-4 !bg-white/50">
                        <ResponsiveContainer width="100%" height={300}>
                            <LineChart data={projectionData}>
                                ...
                                />
                                <Legend iconType="circle" wrapperStyle={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 800, paddingTop: '20px' }} />
                                <Line type="monotone" dataKey="income" stroke="#4f46e5" strokeWidth={3} dot={{ r: 4, fill: '#4f46e5', strokeWidth: 2, stroke: '#fff' }} name="Income" />
                                <Line type="monotone" dataKey="tax" stroke="#ef4444" strokeWidth={3} dot={{ r: 4, fill: '#ef4444', strokeWidth: 2, stroke: '#fff' }} name="Tax" />
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                        {projectionData.map((year) => (
                            <div key={year.year} className="ios-card bg-white/40 border-white/50 p-4 transition-all hover:border-indigo-200">
                                <h3 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-3 leading-none">{year.year}</h3>
                                <div className="space-y-3">
                                    <div>
                                        <p className="text-[9px] font-bold text-gray-400 uppercase tracking-tighter leading-none mb-1">Take-Home</p>
                                        <p className="text-xs font-bold text-gray-900 leading-none">{formatCurrency(year.takeHome)}</p>
                                    </div>
                                    <div>
                                        <p className="text-[9px] font-bold text-gray-400 uppercase tracking-tighter leading-none mb-1">Eff. Rate</p>
                                        <p className="text-xs font-bold text-indigo-600 leading-none">{year.effectiveRate}%</p>
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="bg-indigo-50/50 border-l-4 border-indigo-500 p-5 rounded-xl">
                        <p className="text-[11px] font-medium text-indigo-900 leading-relaxed">
                            <strong className="block mb-1 font-bold uppercase tracking-widest text-[9px]">Growth Analysis</strong>
                            Based on a 10% annual scale-up, your annual tax contribution will adjust from{' '}
                            <span className="font-bold">{formatCurrency(results.annualTax)}</span> to <span className="font-bold">{formatCurrency(projectionData[4].tax)}</span> by 2030.
