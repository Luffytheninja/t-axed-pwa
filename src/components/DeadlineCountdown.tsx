...
export default function DeadlineCountdown() {
    ...
    return (
        <div className={`rounded-xl p-4 flex items-center justify-between shadow-sm border ${isUrgent ? 'bg-red-50 border-red-100' : 'bg-gray-50 border-gray-200'} mb-6`}>
            <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isUrgent ? 'bg-red-100 text-red-600' : 'bg-gray-200 text-gray-600'}`}>
                    {isUrgent ? <AlertTriangle className="w-5 h-5" /> : <Timer className="w-5 h-5" />}
                </div>
                <div>
                    <span className={`text-[10px] font-bold uppercase tracking-widest block ${isUrgent ? 'text-red-500' : 'text-gray-400'}`}>Filing Deadline</span>
                    <span className={`text-sm font-bold ${isUrgent ? 'text-red-900' : 'text-gray-900'}`}>March 31st Compliance</span>
                </div>
            </div>
