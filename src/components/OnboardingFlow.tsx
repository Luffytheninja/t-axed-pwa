            ...
            <div className="max-w-md w-full space-y-12 mb-20">
                ...
                {step === 3 && (
                    <div className="view-transition space-y-8">
                        ...
                        <div>
                            <h2 className="text-3xl font-black text-gray-900 tracking-tight mb-4">Profile Identity</h2>
                            <p className="text-gray-500 font-medium">Identify your earner classification</p>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            {[
                                { id: 'PAYE', label: 'Salary (PAYE)' },
                                { id: 'SmallBusiness', label: 'Small Biz' },
                                { id: 'Professional', label: 'Freelancer' },
                                { id: 'DigitalNomad', label: 'Digital Nomad' }
                            ].map((cat) => (
                                <button
                                    key={cat.id}
                                    onClick={() => setCategory(cat.id as UserCategory)}
                                    className={`p-6 rounded-2xl border-2 transition-all font-bold text-sm ${category === cat.id ? 'bg-purple-600 border-purple-600 text-white shadow-lg' : 'bg-white border-gray-100 text-gray-900'
                                        }`}
                                >
                                    {cat.label}
                                </button>
                        ...
                        <button
                            onClick={nextStep}
                            className="ios-btn-primary w-full h-16 text-lg bg-purple-600 hover:bg-purple-700"
                        >
